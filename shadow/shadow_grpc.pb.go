...
type ShadowServiceClient interface {
	...
	StreamShadow(ctx context.Context, in *StreamShadowRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Shadow], error)
}
...
func (c *shadowServiceClient) StreamShadow(ctx context.Context, in *StreamShadowRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Shadow], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ShadowService_ServiceDesc.Streams[0], ShadowService_StreamShadow_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[StreamShadowRequest, Shadow]{ClientStream: stream}
...
type ShadowService_StreamShadowClient = grpc.ServerStreamingClient[Shadow]
...
type ShadowServiceServer interface {
	...
	StreamShadow(*StreamShadowRequest, grpc.ServerStreamingServer[Shadow]) error
...
func (UnimplementedShadowServiceServer) StreamShadow(*StreamShadowRequest, grpc.ServerStreamingServer[Shadow]) error {
...
func _ShadowService_StreamShadow_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ShadowServiceServer).StreamShadow(m, &grpc.GenericServerStream[StreamShadowRequest, Shadow]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ShadowService_StreamShadowServer = grpc.ServerStreamingServer[Shadow]
